  ...
  return (
    <Card variant={cardVariant} className="min-w-[180px]">
      ...
      <div className="flex items-baseline gap-1.5 mb-2">
        <span
          className={`text-4xl font-bold tabular-nums transition-colors duration-300 ${valueColorClass}`}
        >
          {value.toFixed(1)}
        </span>
      ...
      {/* Status Badge */}
      <div
        className={`inline-block px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-wider ${statusBadgeClass}`}
      >
        {statusText}
      </div>
    </Card>
