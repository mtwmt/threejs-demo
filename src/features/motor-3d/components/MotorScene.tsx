...
import { getQualitySettings, FPSMonitor, type DeviceCapability } from '../utils';
...
export function MotorScene({
  ...
  // 平滑處理用的歷史數據
  const smoothedAnglesRef = useRef<JointAngles>({ j1: 0, j2: 0, j3: 0, j4: 0, j5: 0, j6: 0 });
  const smoothedSensorRef = useRef<SensorData>({ jointLoad: 30, speed: 200, cycleTime: 12 });
   ...
   */
  const calculateSensorData = useCallback((currentAngles: JointAngles): SensorData => {
    ...
    if (prevAngles) {
      ...
      // 過濾大跳變
      totalAngularVelocity = changes.filter(c => c < 30).reduce((a, b) => a + b, 0);
    }
    ...
    const smoothed: SensorData = {
      jointLoad: Math.round(prev.jointLoad + smoothingFactor * (rawLoad - prev.jointLoad)),
      speed: Math.round(prev.speed + smoothingFactor * (rawSpeed - prev.speed)),
      cycleTime: 12.0,
    };
  ...
  const initScene = useCallback(() => {
    ...
    setQualityLevel(qualitySettings.pixelRatio > 1.5 ? 'high' : qualitySettings.pixelRatio > 1 ? 'medium' : 'low');
    ...
    loader.load(
      ...
      (gltf) => {
        ...
        const jointKeywords = ['joint', 'arm', 'link', 'shoulder', 'elbow', 'wrist', 'axis', 'rotate'];
        ...
        console.log(`Found ${allBones.length} total bones, ${jointBones.length} joint-named bones`);
        console.log('Selected bones:', bonesRef.current.map(b => b.name));
  ...
  }, [isPaused, onJointAnglesUpdate, onSensorDataUpdate, extractJointAngles, calculateSensorData]);
    ...
    <div className="relative w-full h-full rounded-xl overflow-hidden bg-gradient-to-br from-[#0a0a0f] to-[#1a1a2e]">
      {loadingStatus === 'loading' && (
        <div className="absolute inset-0 flex items-center justify-center z-10">
          <div className="text-white/60 text-lg animate-pulse">載入模型中...</div>
        </div>
      )}
      ...
      <div className="absolute top-0 left-0 right-0 z-10 pointer-events-none">
        <div className="flex items-center justify-between px-4 py-3 bg-gradient-to-b from-black/60 to-transparent">
          ...
          <div className="flex items-center gap-4 text-sm">
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/5 backdrop-blur-sm border border-white/10">
              ...
              <span className={`font-mono font-medium ${
                (sensorData?.jointLoad ?? 30) > 70 ? 'text-red-400' :
                (sensorData?.jointLoad ?? 30) > 50 ? 'text-yellow-400' :
                'text-green-400'
              }`}>
          ...
          <div className="flex items-center gap-2">
            <div className={`px-2 py-1 rounded text-xs font-mono backdrop-blur-sm ${
              fps >= 50 ? 'bg-green-500/20 text-green-400' : 
            ...
            <div className={`px-2 py-1 rounded text-xs font-mono backdrop-blur-sm ${
              qualityLevel === 'high' ? 'bg-purple-500/20 text-purple-400' :
              qualityLevel === 'medium' ? 'bg-blue-500/20 text-blue-400' :
      ...
      <div className="absolute bottom-4 left-4 z-10 pointer-events-none">
        <div className="flex flex-col gap-1 px-3 py-3 rounded-xl bg-black/40 backdrop-blur-sm border border-white/10 text-xs font-mono">
          ...
          <div className="text-white/70">
            ...
            ({cameraDebug.position.x}, {cameraDebug.position.y}, {cameraDebug.position.z})
          </div>
          <div className="text-white/70">
            <span className="text-white/50">target: </span>
            ({cameraDebug.target.x}, {cameraDebug.target.y}, {cameraDebug.target.z})
          </div>
      ...
      {/* 3D Model Canvas */}
      <div ref={containerRef} className="w-full h-full [&_canvas]:block [&_canvas]:rounded-xl" />
